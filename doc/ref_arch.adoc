...
Instead, the domain model jar needs to be made visible on the classpath of the BPM Suite business-central web artifact.

Since this reference architecture does show-case use of invoking a remote SOAP service that requires a complex-type in its payload, the domain model jar will be made visible to the business-central web archive. 
...
Previously we've discussed the requirement to add this domain model library to the business-central.war/WEB-INF/lib directory.

BPM Suite 6 includes a _Data Modeler_ tool.
The Data Modeler tool allows for the creation of domain model classes.
Classes created through the BPM Suite 6 Data Modeler should be considered a _processTier_ domain model only.
Specifically, this _processTier_ domain model is useful at design-time to facilate creation of rules, BPMN2 process definitions, forms, etc.
It is also useful at runtime to facilitate the execution of rules and process instances included in a KIE project.
In summary, the scope of this _processTier_ domain model is specific to KIE workbench tooling and KIE project execution.
...
When integration between a process instance and SOA services are needed, it is a best practice to define an additional _integration_ domain model.
