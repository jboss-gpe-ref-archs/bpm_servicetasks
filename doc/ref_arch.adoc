...
Both BPM Suite 6 SOAP-based WorkItemHandlers leverage Apache CXF Dynamic Dispatch to invoke remote SOAP services.
...
BPM Suite 6 does not include a supported out-of-the-box _Executor Service_.
A community supported _Executor Service_ is available however from the open-source jbpm project.

This reference architecture provides a working example of an Asynchroneous Service Task invoking the jbpm Executor Service.
